...
type TimeSlotsComponentProps = {
  ...
  timeSlots: { date: string; slots: ConvertedSlot[] }[];
...
export default function TimeSlotsComponent({
  ...
  const SelectedSlote = timeSlots
    .filter((day) => {
      // Only match the selected date (no time check)
      const isSelectedDate =
        day.date === format(selectedDate || today, "yyyy-MM-dd");
      return isSelectedDate;
    })
    .filter((day) => day?.slots.length > 0);

  useEffect(() => {
    if (SelectedSlote.length === 0) {
      setShakeKey((k) => k + 1);
    }
  }, [SelectedSlote.length, selectedDate]);
